...
def generateEdges(nodes,Max_Range):
...
def readedges(nodes,Max_Range):
    try:
        df = pd.read_csv("D:\Assignments\Algo\impl\lora_graph_gw\Pre_Study\Pmedian\savededges"+str(Max_Range)+".csv",sep=",")
...
def saveedges(edges,Max_Range):
    ...
    for curr in edges:            
        ...
        idx +=1
    pd.DataFrame(data=output).to_csv("D:\Assignments\Algo\impl\lora_graph_gw\Pre_Study\Pmedian\savededges"+str(Max_Range)+".csv")
    pass
...
def create_placement(vars):
    Max_Range=vars[0]
    Sensors=vars[1]
    distances=generateEdges(Sensors,Max_Range)
    # Sensors=[(1, 2), (3, 5), (7, 8), (9, 10)]
    n=len(Sensors)
    num_facilities=vars[2]
    ...
    for i in Sensors:
        for j in facility_locations:
            prob += distance(i, j) * assignment[(i, j)] <= Max_Range
    ...
    for i in Sensors:
        ...
        for g in Gateways:
            ...
            # jui print(g)
    pd.DataFrame(data=output).to_csv('D:\Assignments\Algo\impl\lora_graph_gw\Pre_Study\Pmedian/files\S1\gateways_Placement_Range'+str(int(Max_Range))+'.csv')
    ...
    pd.DataFrame(data=output).to_csv('D:\Assignments\Algo\impl\lora_graph_gw\Pre_Study\Pmedian/files\S1/reachable_sensors_Range'+str(int(Max_Range))+'.csv')
    pd.DataFrame(data=output).to_json('D:\Assignments\Algo\impl\lora_graph_gw\Pre_Study\Pmedian/files\S1/reachable_sensors_Range'+str(int(Max_Range))+'.json')
    ...
    with open("D:\Assignments\Algo\impl\lora_graph_gw\Pre_Study\Pmedian\sensors.csv", newline='') as csvfile:
        ...
        for curr in data:
            try:
                ...
                pass
    number_gw=100
    create_placement([1150,Sensors,number_gw])
